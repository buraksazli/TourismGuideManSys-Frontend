...
function BanButton({username}) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault(); 
        try {
          const response = await banUser(username , numberOfDay);
          
          if(!response.succeeded)
            console.log(response.response.data.errors)
          console.log(response);     
        } catch (error){
            setError('Adding not successful');console.log(username);
            console.log(error);
        }
    };
  ...
  return (
    <div>
        <Button variant="btn btn-danger" onClick={handleShow}>
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-shield-fill-x" viewBox="1 0 18 19">
          ...
          <Modal.Body style={{backgroundColor: "#DCDCDC"}}>
            ...
            <form  onSubmit={handleSubmit}>
                <div className="form-group mt-3 "> 
                    <input type="text" className="form-control h-100 " name='firstName'  value={numberOfDay} onChange={(e) => setNumberOfDay(e.target.value)} placeholder="Enter Ban Day"></input>
                </div>
